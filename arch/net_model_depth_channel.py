...
class net_model_depth_channel(torch.nn.Module):
    
    def __init__(self,base_channel=32):
        
        super(net_model_depth_channel, self).__init__()

        print(base_channel)
        
        self.layer_01 = torch.nn.Sequential(
            torch.nn.ConvTranspose2d(
                1, base_channel, (3, 3), stride = (1, 1), padding = 1
                ),
            torch.nn.BatchNorm2d(base_channel),
            torch.nn.LeakyReLU(),
            
            conv_block(in_channels=base_channel,out_channels=base_channel)
            )
        ...
        self.layer_02 = conv_block(in_channels=base_channel,out_channels=base_channel*2)
        ...
        self.layer_03 = conv_block(in_channels=base_channel*2,out_channels=base_channel*4)
        ...
        self.layer_04 = conv_block(in_channels=base_channel*4,out_channels=base_channel*8)
        ...
        self.layer_05 = conv_block(in_channels=base_channel*8,out_channels=base_channel*16)
        ...
        self.layer_06_01 = torch.nn.Sequential(
            torch.nn.ConvTranspose2d(
                base_channel*16, base_channel*8, (3, 3), stride = (2, 2), padding = 1, 
                output_padding = 1
                ),
            torch.nn.BatchNorm2d(base_channel*8),
            torch.nn.LeakyReLU()
            )
            
        self.layer_06_02 = conv_block(in_channels=base_channel*16,out_channels=base_channel*8)
        ...
        self.layer_07_01 = torch.nn.Sequential(
            torch.nn.ConvTranspose2d(
                base_channel*8, base_channel*4, (3, 3), stride = (2, 2), padding = 1, 
                output_padding = 1
                ),
            torch.nn.BatchNorm2d(base_channel*4),
            torch.nn.LeakyReLU()
            )
            
        self.layer_07_02 = conv_block(in_channels=base_channel*8,out_channels=base_channel*4)
        
        # layer_08
        
        self.layer_08_01 = torch.nn.Sequential(
            torch.nn.ConvTranspose2d(
                base_channel*4, base_channel*2, (3, 3), stride = (2, 2), padding = 1, 
                output_padding = 1
                ),
            torch.nn.BatchNorm2d(base_channel*2),
            torch.nn.LeakyReLU()
            )
            
        self.layer_08_02 = conv_block(in_channels=base_channel*4,out_channels=base_channel*2)
        
        # layer_09
        
        self.layer_09_01 = torch.nn.Sequential(
            torch.nn.ConvTranspose2d(
                base_channel*2, base_channel, (3, 3), stride = (2, 2), padding = 1, 
                output_padding = 1
                ),
            torch.nn.BatchNorm2d(base_channel),
            torch.nn.LeakyReLU()
        ...
        self.layer_09_02 = conv_block(in_channels=base_channel*2,out_channels=base_channel)
        
        # layer_10
        
        self.layer_10 = torch.nn.Sequential(
            torch.nn.Conv2d(base_channel, 1, (3, 3), stride = (1, 1), padding = 1),
...
if __name__=='__main__':
    ...
    net = net_model_depth_channel(base_channel=64)
    y = net(x)
    print(y.shape)
    print(net)
