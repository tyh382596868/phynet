...
if __name__ == "__main__":
    ...
    gt_matrix_raw = my_readtxt(gt_txt)

    gt_matrix = cv2.resize(gt_matrix_raw, (1024,1024))
    ...
    for loop in tqdm(loops):
        ...
        tb_folder = f'{result_folder}/tb_folder/{loop}'

        weight_folder = f"{result_folder}/weight_folder/{loop}"
        img_txt_folder = f'{result_folder}/img_txt_folder/{loop}'
        ...
        data_raw = np.loadtxt(measured_y_path_txt,dtype=np.float32,delimiter=",") # frame:文件
        data = cv2.resize(data_raw, (1024,1024))
        ...
        for current_epoch in tqdm(range(epochs)):
            ...
            for batch,(x,y) in (enumerate(train_dataloader)):
                ...
                measured_y2 = prop(pred_y2[0, 0, :, :],dist=para.dist)

                # cross entropy loss for classifier
                ce_loss = 0.5 * (loss_mse(y.float(),measured_y.float()) + loss_mse(y.float(),measured_y2.float()))

                kl_loss = compute_kl_loss(pred_y, pred_y2)

                # carefully choose hyper-parameters
                loss_value = ce_loss + loop * kl_loss  
                ...
                if step % 50 == 0:
                    ...
                    phase_diff = np.abs(gt_matrix-((pred_y).cpu().detach().numpy().reshape(1024,1024)))
                ...
                # 记录中间结果图片
                if step % 3000 == 0:
                            my_saveimage(pred_y.cpu().detach().numpy().reshape(1024,1024),f'{img_txt_folder}/{step}_pred.png')
                            my_savetxt(pred_y.cpu().detach().numpy().reshape(1024,1024),f'{img_txt_folder}/{step}_pred.txt')

                            my_saveimage(measured_y.cpu().detach().numpy().reshape(1024,1024),f'{img_txt_folder}/{step}_measured_y.png')
                            my_savetxt(measured_y.cpu().detach().numpy().reshape(1024,1024),f'{img_txt_folder}/{step}_measured_y.txt')

                            my_saveimage((x-measured_y).cpu().detach().numpy().reshape(1024,1024),f'{img_txt_folder}/{step}_{name}-measured_y.png')
                            my_savetxt((x-measured_y).cpu().detach().numpy().reshape(1024,1024),f'{img_txt_folder}/{step}_{name}-measured_y.txt')
                            
                            my_saveimage(gt_matrix-((pred_y).cpu().detach().numpy().reshape(1024,1024)),f'{img_txt_folder}/{step}_gt-_pred.png')
                            my_savetxt(gt_matrix-((pred_y).cpu().detach().numpy().reshape(1024,1024)),f'{img_txt_folder}/{step}_gt-_pred.txt')
