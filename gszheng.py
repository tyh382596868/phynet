...
if __name__ == '__main__':
    ...
    Uo = amp_gt*torch.exp(1j*init_phase).to(device) #光纤端面初始复光场
    ...
    for epoch1 in tqdm(range(epoch1s)):
        ...
        for i in range(1):
            Ui = propcomplex(Uo,dist = zs,device=device)
            Ua = A[:,:,i]*Ui/torch.abs(Ui)
            Um = propcomplex(Ua,dist = -zs,device=device)
            Uo = amp_gt*Um/torch.abs(Um)
    ...
    my_saveimage((pha-phase_ref.cpu().detach().numpy()),f'{save_path}/{epoch1s}_refloss_pi.png')
    my_savetxt((pha-phase_ref.cpu().detach().numpy()),f'{save_path}/{epoch1s}_refloss_pi.txt')
