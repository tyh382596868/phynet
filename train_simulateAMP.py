    ...
    for current_epoch in tqdm(range(epochs)):
        for i, (Speckle,Pha) in enumerate(dataloader):
            ...
            if step % 300 == 0 or step+1 == epochs:
                        dpi = 800
                        my_saveimage(np.mod(flattened_pred_pha.cpu().detach().numpy(),2*np.pi),f'{img_txt_folder}/{step}_PredPha.png',dpi=dpi)
                        my_savetxt(np.mod(flattened_pred_pha.cpu().detach().numpy(),2*np.pi),f'{img_txt_folder}/{step}_PredPha.txt')
