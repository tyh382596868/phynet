...
class RandomResizeFromSequence(object):
    ...
    @staticmethod
    def get_params(resizes):
        """Get parameters for ``rotate`` for a random rotation.
        Returns:
            sequence: params to be passed to ``rotate`` for random rotation.
        """
        resize = random.choice(resizes)
        return resize
    ...
    def __call__(self, data):
        ...
        """
        hr, lr = data
        resize = self.get_params(self.resizes)
        return F.resize(hr, (resize)), \
                F.resize(lr, (resize))
...
class RandomRotationFromSequence(object):
    ...
    def __call__(self, data):
        ...
        return F.rotate(hr, int(angle), self.resample, self.expand, self.center), \
                F.rotate(lr, int(angle), self.resample, self.expand, self.center)
