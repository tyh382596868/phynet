# CONFIG_FILE="/mnt/data/optimal/tangyuhang/workspace/iopen/ai4optical/phynet_git/phynet/option/ref_baseline.yaml"
# python train.py --opt $CONFIG_FILE

# CONFIG_FILE="/mnt/data/optimal/tangyuhang/workspace/iopen/ai4optical/phynet_git/phynet/option/sam_net_model_depth.yaml"
# python train.py --opt $CONFIG_FILE

# ref_net_model_res,net_model_Dropout,net_model_GroupNorm,net_model_Dropout_full
# CONFIG_FILE="/mnt/data/optimal/tangyuhang/workspace/iopen/ai4optical/phynet_git/phynet/option/net_model_Dropout_full.yaml"
# python train.py --opt $CONFIG_FILE

# CONFIG_FILE="/mnt/data/optimal/tangyuhang/workspace/iopen/ai4optical/phynet_git/phynet/option/ref_lr.yaml"
# python train_lr.py --opt $CONFIG_FILE

# CONFIG_FILE="/mnt/data/optimal/tangyuhang/workspace/iopen/ai4optical/phynet_git/phynet/option/ref_channel.yaml"
# python train_channel.py --opt $CONFIG_FILE

# CONFIG_FILE="/mnt/data/optimal/tangyuhang/workspace/iopen/ai4optical/phynet_git/phynet/option/ref_loss.yaml"
# python train_loss.py --opt $CONFIG_FILE


# CONFIG_FILE="/mnt/data/optimal/tangyuhang/workspace/iopen/ai4optical/phynet_git/phynet/option/R_Droup.yaml"
# python train_R_Drop.py --opt $CONFIG_FILE

# CONFIG_FILE="/mnt/data/optimal/tangyuhang/workspace/iopen/ai4optical/phynet_git/phynet/option/net_model_Dropout/net_model_Dropout_p.yaml"
# CONFIG_FILE="/mnt/data/optimal/tangyuhang/workspace/iopen/ai4optical/phynet_git/phynet/option/net_model_Dropout_full/net_model_Dropout_full_last_p.yaml"
CONFIG_FILE="/mnt/data/optimal/tangyuhang/workspace/iopen/ai4optical/phynet_git/phynet/option/net_model_Dropout_full/net_model_Dropout_full_all_p.yaml"
python /mnt/data/optimal/tangyuhang/workspace/iopen/ai4optical/phynet_git/phynet/option/net_model_Dropout/train_net_model_Dropout_p.py --opt $CONFIG_FILE

# CONFIG_FILE="/mnt/data/optimal/tangyuhang/workspace/iopen/ai4optical/phynet_git/phynet/option/net_model_GroupNorm/net_model_GroupNorm_group.yaml"
# python /mnt/data/optimal/tangyuhang/workspace/iopen/ai4optical/phynet_git/phynet/option/net_model_GroupNorm/train_net_model_GroupNorm_group.py --opt $CONFIG_FILE