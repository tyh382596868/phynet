...
if __name__ == "__main__":
    ...
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu') #分布式训练时用默认
    ...
    mask_txt = '/mnt/data/optimal/tangyuhang/workspace/iopen/ai4optical/phynet_git/traindata/gt/1536_1536_mask.txt'
    mask = my_readtxt(mask_txt)
    gt_matrix = gt_matrix*mask
    mask = torch.tensor(mask).to(device)

    print(gt_matrix.dtype)

    result_folder = f'/mnt/data/optimal/tangyuhang/workspace/iopen/ai4optical/phynet_git/result/{name}_mask/{localtime}'
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=5000, gamma=0.90)
...
# '''
    ...
    for current_epoch in tqdm(range(epochs)):
        ...
        for batch,(x,y) in (enumerate(train_dataloader)):
            ...
            pred_y[0, 0, :, :] = pred_y[0, 0, :, :]*mask
            


            measured_y = prop(pred_y[0, 0, :, :],dist=0.01,device=device)
